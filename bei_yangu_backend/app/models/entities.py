...
class Hospital(Base):
    ...
    tier = Column(String(50), default="Level 5")
    ownership = Column(String(50), default="private")
    rating = Column(Float, default=4.0)
    review_count = Column(Integer, default=0)
    image_url = Column(String(500), default="")

    services = relationship(
        "HospitalService", back_populates="hospital", cascade="all, delete-orphan"
    )
    equipment_links = relationship(
        "HospitalEquipment", back_populates="hospital", cascade="all, delete-orphan"
    )
    insurance_links = relationship(
        "HospitalInsurance", back_populates="hospital", cascade="all, delete-orphan"
    )
...
class Procedure(Base):
    ...
    services = relationship(
        "HospitalService", back_populates="procedure", cascade="all, delete-orphan"
    )
    synonyms = relationship(
        "ProcedureSynonym", back_populates="procedure", cascade="all, delete-orphan"
    )
...
class HospitalService(Base):
    ...
    procedure_id = Column(
        Integer, ForeignKey("procedures.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class InsuranceProvider(Base):
    ...
    hospital_links = relationship(
        "HospitalInsurance", back_populates="insurance", cascade="all, delete-orphan"
    )
...
class HospitalInsurance(Base):
    ...
    __table_args__ = (
        UniqueConstraint("hospital_id", "insurance_id", name="uq_hospital_insurance"),
    )
    ...
    insurance_id = Column(
        Integer, ForeignKey("insurance_providers.id", ondelete="CASCADE"), nullable=False
    )
...
class Equipment(Base):
    ...
    hospital_links = relationship(
        "HospitalEquipment", back_populates="equipment", cascade="all, delete-orphan"
    )
...
class HospitalEquipment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("hospital_id", "equipment_id", name="uq_hospital_equipment"),
    )
