...
def _fallback_summary(query: str, procedures: list[Procedure]) -> str:
    if not procedures:
        return (
            ...
            "Try searching by procedure name (e.g. 'normal delivery', 'appendectomy', 'CT scan')."
        )
...
KEYWORD_MAP: dict[str, list[str]] = {
    ...
    "brain": ["ct-scan", "mri-scan"],
    "knee": ["knee-arthroscopy"],
    "eye": ["cataract-surgery"],
    "teeth": ["dental-extraction", "root-canal"],
    "heart": ["echocardiogram", "ecg"],
    "kidney": ["dialysis", "kidney-biopsy"],
    "cancer": ["biopsy", "ct-scan"],
    "blood": ["full-blood-count", "malaria-test"],
    "malaria": ["malaria-test"],
    "tb": ["chest-xray", "sputum-test"],
    "tuberculosis": ["chest-xray", "sputum-test"],
    "diabetes": ["blood-sugar-test", "hba1c"],
    "back": ["mri-scan", "xray-spine"],
    "joint": ["mri-scan", "xray"],
...
def ai_search(db: Session, query: str) -> AISearchResponse:
    ...
    catalogue = [
        {"slug": p.slug, "name": p.name, "category": p.category} for p in all_procedures
    ]
...
If nothing fits well, return empty procedure_slugs and explain in summary."""
    ...
    if not suggested:
        ...
        for p in all_procedures:
            if q_lower in p.name.lower() or q_lower in p.slug.replace("-", " "):
                suggested.append(p)
